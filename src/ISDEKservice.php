...
ISDEKservice::setTarifPriority(
  array(231, 139, 124, 121, 118, 61, 60, 59, 58, 57, 8, 3, 1),
  array(376, 366, 361, 232, 138, 126, 123, 120, 12)
);
  ...
  public static function getCityByAddress($address)
  {
    ...
    if (isset($cdekCity['error'])) {
      ...
      if (count($cdekCity['cities']) > 0) {
        ...
        $arPretend = array();
        // parseCountry
        if (isset($arStages['country']) && $arStages['country']) {
        ...
        if (count($arPretend) > 1) {
          if (isset($arStages['region']) && $arStages['region']) {
            $_arPretend = array();
            foreach ($arPretend as $arCity) {
              $possRegion = str_replace(self::getRegion(), '', mb_strtolower(trim($arCity['region'])));
              if (!$possRegion || mb_stripos(
                $possRegion,
                str_replace(self::getRegion(), '', $arStages['region'])
              ) !== false) {
                $_arPretend[] = $arCity;
              }
            }
        ...
        // parseSubRegion
        if (count($arPretend) > 1) {
          if (isset($arStages['subregion']) && $arStages['subregion']) {
            $_arPretend = array();
            foreach ($arPretend as $arCity) {
    ...
    if ($request && $request['code'] == 200) {
      ...
      foreach ($xml as $key => $val) {
        ...
        $arList[$type][$cityCode][$code] = array(
          ...
          'Postamat'       => (strtolower($val['Type']->__toString()) == 'postomat'),
