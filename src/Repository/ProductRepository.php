...
class ProductRepository extends BaseProductRepository
{
    ...
    public function findLatestByChannel(ChannelInterface $channel, string $locale, int $count): array
    {
        return $this->createQueryBuilder('o')
            ...
            ->andWhere(':channel MEMBER OF o.channels')
            ->andWhere('o.enabled = true')
            ->addSelect('MAX(CASE WHEN attribute.code = :code THEN attributeValue.integer ELSE 0 END) AS HIDDEN publishDateOrder')
            ->groupBy('o.id')
            ->addOrderBy('publishDateOrder', 'DESC')
    ...
    /**
     * Find latest products by taxon (includes products from all descendant taxons)
     */
    public function findLatestByTaxon(ChannelInterface $channel, TaxonInterface $taxon, string $locale, int $count): array
    {
        return $this->createQueryBuilder('o')
            ->addSelect('translation')
            ->innerJoin('o.translations', 'translation', 'WITH', 'translation.locale = :locale')
            ...
            ->andWhere('o.enabled = true')
            ->groupBy('o.id')
            ->addSelect('MAX(CASE WHEN attribute.code = :attributeCode THEN attributeValue.integer ELSE 0 END) AS HIDDEN publishDateOrder')
            ->addOrderBy('publishDateOrder', 'DESC')
            ->addOrderBy('o.createdAt', 'DESC')
            ->setParameter('channel', $channel)
            ->setParameter('locale', $locale)
