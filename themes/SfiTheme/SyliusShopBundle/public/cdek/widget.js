  ...
  var widjet = new ipolWidjetController({
    ...
    options: {
      ...
      goods: {
        ...
        setting: "start",
        hint:
          "Value must be an array of objects of type {length:(float),width:(float),height(float),weight(float)}",
      },
    ...
    PVZ: {
      ...
      getCurrent: function () {
        if (widjet.options.get("region"))
          return this.getRegionPVZ(DATA.city.current);
    ...
    html: {
      ...
      place: function () {
        ...
        if (widjet.options.get("hidedress")) {
          var temp = [];
          temp.push(
            htmlka.slice(0, widjet.options.get("hidecash") ? 132 : 1349)
          );
      ...
      open: function () {
        ...
        if (
          ipjq(IDS.get("cdek_widget_cnt")).find(
            ".CDEK-widget__search-list__box li"
          ).length >= 10
        ) {
          ipjq(IDS.get("cdek_widget_cnt"))
            .find(".CDEK-widget__search-list__box")
            .mCustomScrollbar();
        }
      },
          ...
          ymaps.geolocation.get({}).then(function (result) {
            ...
            for (var i = gdeUser.length - 1; i >= 0; i--) {
              if (gdeUser[i].kind == "locality") {
                city = gdeUser[i].name;
                city = city.replace(
                  /РіРѕСЂРѕРґ\s|РїРѕСЃРµР»РѕРє\sРіРѕСЂРѕРґСЃРєРѕРіРѕ\sС‚РёРїР°\s|РїРѕСЃРµР»РѕРє\s|РїРѕСЃС‘Р»РѕРє\s|РґРµСЂРµРІРЅСЏ\s|СЃРµР»Рѕ\s/gi,
                  ""
                );
                DATA.city.set(city);
      ...
      placeMarks: function (mtypes) {
        ...
        if (typeof pvzList === "object") {
          ...
          for (var i in pvzList) {
            ...
            pvzList[i].placeMark.events.add(["balloonopen", "click"], function (
              metka
            ) {
      ...
      .on("keydown", ".CDEK-widget__search-box input[type=text]", function (e) {
        ...
        if (e.keyCode === 40) {
          if ($liFocus.length == 0) {
            ...
            ipjq(IDS.get("cdek_widget_cnt"))
              .find(".CDEK-widget__search-list__box")
              .mCustomScrollbar("scrollTo", $liActive.first(), {
                scrollInertia: 300,
              });
          } else {
            ...
            if (
              ...
              ipjq(IDS.get("cdek_widget_cnt"))
                .find(".CDEK-widget__search-list__box")
                .mCustomScrollbar("scrollTo", $liFocus.next($liActive), {
                  scrollInertia: 300,
                });
            } else {
              $liActive.first().addClass("focus");
        ...
        if (e.keyCode === 38) {
          if ($liFocus.length == 0) {
            ...
            ipjq(IDS.get("cdek_widget_cnt"))
              .find(".CDEK-widget__search-list__box")
              .mCustomScrollbar("scrollTo", $liActive.last(), {
                scrollInertia: 300,
              });
          } else {
            $liFocus.removeClass("focus");
            if (
              ...
              ipjq(IDS.get("cdek_widget_cnt"))
                .find(".CDEK-widget__search-list__box")
                .mCustomScrollbar("scrollTo", $liFocus.prev($liActive), {
                  scrollInertia: 300,
                });
            } else {
              $liActive.last().addClass("focus");
      ...
      .on("keyup", ".CDEK-widget__search-box input[type=text]", function (e) {
        ...
        if (filter != "") {
          $matches = $li.filter(function () {
            return filter.test(
              ipjq(this)
                .find(".CDEK-widget__search-list__city-name")
                .text()
                .replace(/[^\wР°-СЏС‘\s-]+/gi, "")
            );
          });
